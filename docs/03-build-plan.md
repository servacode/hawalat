...
## 6. القرارات المؤكَّدة (2026-08-07) ✅

1. **تعدد المستأجرين:** **قاعدة واحدة + `tenant_id`** مع فرض صارم للعزل على مستوى الاستعلام.
2. **التصميم:** **Tailwind + إعداد توكِنز مركزي** (كل الألوان/الخطوط/الأحجام من مصدر واحد، RTL أولاً).
3. **الهوية البصرية:** **Claude يقترح هوية احترافية** → انظر `04-visual-identity.md`.
4. **اللغة:** **عربي فقط الآن** (RTL)، مع بنية i18n جاهزة لإضافة لغات لاحقاً.

> ✅ كل القرارات محسومة. المسار: المرحلة 0 (أساسات) → المرحلة 1 (النظام المركزي + الهوية).
